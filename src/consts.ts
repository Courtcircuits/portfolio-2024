// Place any global data in this file.
// You can import this data from anywhere in your site by using the `import` keyword.

export const SITE_TITLE = "courtcircuits";
export const SITE_DESCRIPTION =
    "Tristan Radulescu - i write software and play music";
export const SOCIALS = [
    {
        name: "Linkedin",
        link: "https://www.linkedin.com/in/tristan-mihai-radulescu-917859228/",
    },
    {
        name: "Github",
        link: "https://github.com/Courtcircuits",
    },
    {
        name: "Bluesky",
        link: "https://bsky.app/profile/courtcircuit.xyz",
    },
];

export const PGP_KEY = {
    path: "/mihai.asc",
    fingerprint: "cd7815f471de0976b69e37560e03167e7231ce9f70d201012d60e778fd35c6b4"
}

export const TALKS = [
    {
        name: "Making end-to-end encryption user-friendly [FR]",
        to: "https://www.youtube.com/watch?v=fYSy4kzkuiM",
        at: "Polycloud 2025",
    },
    {
        name: " Hacking Dagger to Play StarCraft [EN]",
        to: "https://www.youtube.com/watch?v=iojge2qSAbI",
        at: "Dagger community call",
    }
];

export const CONTRIBUTIONS = [
    {
        name: "Ferris Key [Core Team Member]",
        to: "https://github.com/ferriskey/ferriskey/pulls?q=is%3Apr+author%3ACourtcircuits+is%3Aclosed",
        description: "Building a next-generation IAM written in Rust (mainly worked on realm settings)"
    },
	{
		name: "Hardy [Committer]",
		to: "https://github.com/ricktaylor/hardy/pull/389",
		description: "[IN REVIEW] TLS and devx stuff"
	},
	{
		name: "PYCA [Committer]",
		to: "https://github.com/pyca/cryptography/pull/13759",
		description: "Adding deep-copy support for every key type in the python cryptographic authority library"
	},
    {
        name: "Gleam compiler [Committer]",
        to: "https://github.com/gleam-lang/gleam/pull/4907",
        description: "Better error handling at compiler level written in Rust"

    },
    {
        name: "Scalar [Dependency Maintainer]",
        to: "https://github.com/scalar/scalar",
        description: "Providing a Rust framework agnostic integration for scalar docs"
    },
    {
        name: "Golang ICAL [Committer]",
        to: "https://github.com/arran4/golang-ical/pull/78",
        description: "My first open source PR :)"
    },
    {
        name: "More on Github !!!",
        to: "https://github.com/Courtcircuits",
        description: "Hey just check it I think it's cool"
    }
]

export const PROJECTS = [
    {
        name: "beep.ovh",
        to: "https://beep.ovh",
        description: "an open source discord clone",
    },

    {
        name: "sealci.git",
        to: "https://github.com/dev-sys-do/sealci",
        description: "a minimalist CI engine in rust",
    },
    {
        name: "hackthecrous.com [☠️]",
        to: "https://hackthecrous.com",
        description: "all university food menus in less than a click",
    },
    {
        name: "gists.app [☠️]",
        to: "https://gists.app",
        description: "an end-to-end encrypted script vault",
    },
];

export const HACKATHONS = [
    {
        name: "rust ctf by thales - 3rd place",
        to: "https://www.linkedin.com/posts/s%C3%A9bastien-lejeune-26290a15_hackathon-rust-polytechmontpellier-ugcPost-7275805156326182913-IR_B?utm_source=share&utm_medium=member_desktop",
        description: "12-19-2024",
    },
    {
        name: "'nuit de l'info' - 2nd place",
        to: "https://nuitdelinfo.com",
        description: "12-5-2024",
    },
    {
        name: "act in space montpellier - 1st place",
        to: "https://www.connectbycnes.fr/hackathon-actinspace",
        description: "02-12-2022",
    },
    {
        name: "'nuit de l'info' - 1st place",
        to: "https://minitel-six.vercel.app",
        description: "12-5-2021",
    },
];
export const RESUME_LINK = "/cv.pdf";
